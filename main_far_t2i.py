...
from util.data import JournalDBDataset
...
from models import far_t2i
from engine_far_t2i import train_one_epoch, evaluate
import copy
...
def get_args_parser():
    parser = argparse.ArgumentParser('FAR_T2I training with Diffusion Loss', add_help=False)
    parser.add_argument('--batch_size', default=16, type=int,
                        help='Batch size per GPU (effective batch size is batch_size * # gpus')
    parser.add_argument('--epochs', default=400, type=int)

    # Model parameters
    parser.add_argument('--model', default='far_t2i', type=str, metavar='MODEL',
    ...
    parser.add_argument('--num_machine', default=1, type=int)
    parser.add_argument('--num_gpus_permachine', default=2, type=int)

    # caching latents
    parser.add_argument('--use_cached', action='store_true', dest='use_cached', help='Use cached latents')
    parser.add_argument('--use_unictrol', action='store_true', dest='use_unictrol', help='Use cached latents')
...
def main(args):
    ...
    model = far_t2i.__dict__[args.model](
    ...
    # Load  dataset
    dataset_train = JournalDBDataset(args.data_path, img_size=args.img_size)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(
            text_tokenizer, text_model, llm_system_prompt,
            model, vae,
        ...
        if args.online_eval and (epoch % args.eval_freq == 0 or epoch + 1 == args.epochs):
            torch.cuda.empty_cache()
            evaluate(text_tokenizer, text_model, llm_system_prompt, model_without_ddp, vae, ema_params, args, epoch, batch_size=args.eval_bsz, log_writer=log_writer, cfg=args.cfg, use_ema=True)
            torch.cuda.empty_cache()
